...
httpReq.onreadystatechange = function(){
    if(httpReq.readyState === 4 && httpReq.status === 200){
        ...
        let cont = resp.map(e=> e.Country);/*---- stock list of contries in array ----- */
        for(let i = 0; i<cont.length; i++)
        {
            let myDev = document.createElement("div");
            myDev.innerHTML = cont[i];
            myDev.setAttribute('id', 'contries');/*--------creat new element HTML for organize liste of contries----*/
...
httpReq.send();/*---------Envoi la reqet--------- */

function  divClicked(e){/*---------------------->>> retourne le nome de pays cliqué et organisé le graphe----*/
